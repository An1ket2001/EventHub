...
const LoginSignup = () => {  
  ...
  useEffect(()=>{
    ...
    const fetchdata=async()=>{
      const data=await fetch("http://localhost:5000/auth/getdesg");
      const real_data=await data.json();
      console.log(real_data);
      setstoredes(real_data);
    }
    fetchdata();
  },[]);
      ...
      <div className="inputs">  
        ...
        {action === "Sign Up" && (  
          <div className="input">  
            ...
            <option value="set_des">Select Designation</option>
            {
              storedes.map((e)=>{
                return <option key={e._id} value={e._id}>{e.designation}</option>  
              })
            }  
            </select>  
